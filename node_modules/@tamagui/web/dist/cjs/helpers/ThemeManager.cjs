...
function getState(props, manager) {
  ...
  const base = baseName.split(import_constants2.THEME_NAME_SEPARATOR),
    max = base.length,
  ...
  for (let i = max; i >= min; i--) {
    ...
    if (process.env.NODE_ENV !== "production" && typeof props.debug == "string" && import_constants.isClient && console.info(` \u{1F537} [${manager?.id}] getState`, {
      ...
      const names = found.split("_"),
        ...
        pre = import_constants2.THEME_CLASSNAME_PREFIX,
        className = import_constants.isWeb ? `${pre}sub_theme ${pre}${!scheme || !restNames.length ? firstName : restNames.join("_")}` : "",
